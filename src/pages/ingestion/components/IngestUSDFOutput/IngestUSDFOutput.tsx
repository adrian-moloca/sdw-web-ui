...
const StyledJsonViewer = styled.div`
  ...
  border-radius: 8px;
`;
export const IngestUSDFOutput = (props: Props) => {
  const apiService = useApiService();
  const url = `${appConfig.apiEndPoint}${appConfig.TRACKING_SYSTEM_INGEST_USDF_Output}/${props.id}`;
  const { data, isLoading } = useQuery({
    queryKey: [props.id, 'usdf_output'],
  ...
  if (!data || data.length === 0)
    return (
      <MainCard>
        <Alert severity="info">{t('messages.no-usdf-data-available')}</Alert>
      </MainCard>
    );
  return (
    <MainCard>
      <Stack direction="row" justifyContent="flex-end" spacing={1} sx={{ my: 1 }}>
        <Button variant="outlined" onClick={openJsonInNewTab} startIcon={<OpenInNewOutlinedIcon />}>
          {t('actions.buttonView')}
        </Button>
        <Button variant="outlined" onClick={downloadJson} startIcon={<FileDownloadOutlinedIcon />}>
