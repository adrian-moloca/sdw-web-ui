...
const QualifiedTeamPage = () => {
  const { getConfig } = useModelConfig();
  const config = getConfig(EntityType.QualifiedTeam);
  ...
  useEffect(() => {
    handleMetadata(EntityType.Team);
  ...
  const urlDownload = `${appConfig.biographiesManagerEndPoint}/qualifiers/teams/download`;
  const mutationDownload = useMutation({
    mutationFn: () =>
      apiService.downloadExport(
        urlDownload,
        `Olympic_Qualified_Teams_${dayjs().format('yyyyMMDD')}.xlsx`
      ),
    onError: (error: any) => {
      if (isDevelopment) Logger.error(error);
      return error;
  ...
  return (
    <DataGridPanel
      ...
      toolbar={[
        ...
        //{ type: GridActionType.EventTypeFilter, values: eventTypeFilter, onChange: (data: any) => setEventTypeFilter(data), visible: true },
      ]}
...
export default QualifiedTeamPage;
